...
// controller reconciling v1.MutatingWebhookConfiguration.
type Interface interface {
	// ReconcileKind implements custom logic to reconcile v1.MutatingWebhookConfiguration. Any changes
	// to the objects .Status or .Finalizers will be propagated to the stored
	// object. It is recommended that implementors do not call any update calls
	// for the Kind inside of ReconcileKind, it is the responsibility of the calling
	// controller to propagate those properties. The resource passed to ReconcileKind
	// will always have an empty deletion timestamp.
	ReconcileKind(ctx context.Context, o *v1.MutatingWebhookConfiguration) reconciler.Event
}

// Finalizer defines the strongly typed interfaces to be implemented by a
// controller finalizing v1.MutatingWebhookConfiguration.
type Finalizer interface {
	// FinalizeKind implements custom logic to finalize v1.MutatingWebhookConfiguration. Any changes
	...
	// deletion timestamp.
	FinalizeKind(ctx context.Context, o *v1.MutatingWebhookConfiguration) reconciler.Event
}

// ReadOnlyInterface defines the strongly typed interfaces to be implemented by a
// controller reconciling v1.MutatingWebhookConfiguration if they want to process resources for which
// they are not the leader.
type ReadOnlyInterface interface {
	// ObserveKind implements logic to observe v1.MutatingWebhookConfiguration.
	// This method should not write to the API.
	ObserveKind(ctx context.Context, o *v1.MutatingWebhookConfiguration) reconciler.Event
}

type doReconcile func(ctx context.Context, o *v1.MutatingWebhookConfiguration) reconciler.Event

// reconcilerImpl implements controller.Reconciler for v1.MutatingWebhookConfiguration resources.
...
func NewReconciler(ctx context.Context, logger *zap.SugaredLogger, client kubernetes.Interface, lister admissionregistrationv1.MutatingWebhookConfigurationLister, recorder record.EventRecorder, r Interface, options ...controller.Options) controller.Reconciler {
...
func (r *reconcilerImpl) updateFinalizersFiltered(ctx context.Context, resource *v1.MutatingWebhookConfiguration) (*v1.MutatingWebhookConfiguration, error) {
	...
	patcher := r.Client.AdmissionregistrationV1().MutatingWebhookConfigurations()
...
func (r *reconcilerImpl) setFinalizerIfFinalizer(ctx context.Context, resource *v1.MutatingWebhookConfiguration) (*v1.MutatingWebhookConfiguration, error) {
...
func (r *reconcilerImpl) clearFinalizer(ctx context.Context, resource *v1.MutatingWebhookConfiguration, reconcileEvent reconciler.Event) (*v1.MutatingWebhookConfiguration, error) {
	if _, ok := r.reconciler.(Finalizer); !ok {
		return resource, nil
	}
	if resource.GetDeletionTimestamp().IsZero() {
