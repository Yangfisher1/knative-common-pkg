...
// controller reconciling v1.ValidatingWebhookConfiguration.
type Interface interface {
	// ReconcileKind implements custom logic to reconcile v1.ValidatingWebhookConfiguration. Any changes
	// to the objects .Status or .Finalizers will be propagated to the stored
	// object. It is recommended that implementors do not call any update calls
	// for the Kind inside of ReconcileKind, it is the responsibility of the calling
	// controller to propagate those properties. The resource passed to ReconcileKind
	// will always have an empty deletion timestamp.
	ReconcileKind(ctx context.Context, o *v1.ValidatingWebhookConfiguration) reconciler.Event
}

// Finalizer defines the strongly typed interfaces to be implemented by a
// controller finalizing v1.ValidatingWebhookConfiguration.
type Finalizer interface {
	// FinalizeKind implements custom logic to finalize v1.ValidatingWebhookConfiguration. Any changes
	...
	// deletion timestamp.
	FinalizeKind(ctx context.Context, o *v1.ValidatingWebhookConfiguration) reconciler.Event
}

// ReadOnlyInterface defines the strongly typed interfaces to be implemented by a
// controller reconciling v1.ValidatingWebhookConfiguration if they want to process resources for which
// they are not the leader.
type ReadOnlyInterface interface {
	// ObserveKind implements logic to observe v1.ValidatingWebhookConfiguration.
	// This method should not write to the API.
	ObserveKind(ctx context.Context, o *v1.ValidatingWebhookConfiguration) reconciler.Event
}

type doReconcile func(ctx context.Context, o *v1.ValidatingWebhookConfiguration) reconciler.Event

// reconcilerImpl implements controller.Reconciler for v1.ValidatingWebhookConfiguration resources.
...
func NewReconciler(ctx context.Context, logger *zap.SugaredLogger, client kubernetes.Interface, lister admissionregistrationv1.ValidatingWebhookConfigurationLister, recorder record.EventRecorder, r Interface, options ...controller.Options) controller.Reconciler {
...
func (r *reconcilerImpl) updateFinalizersFiltered(ctx context.Context, resource *v1.ValidatingWebhookConfiguration) (*v1.ValidatingWebhookConfiguration, error) {
	...
	patcher := r.Client.AdmissionregistrationV1().ValidatingWebhookConfigurations()
...
func (r *reconcilerImpl) setFinalizerIfFinalizer(ctx context.Context, resource *v1.ValidatingWebhookConfiguration) (*v1.ValidatingWebhookConfiguration, error) {
...
func (r *reconcilerImpl) clearFinalizer(ctx context.Context, resource *v1.ValidatingWebhookConfiguration, reconcileEvent reconciler.Event) (*v1.ValidatingWebhookConfiguration, error) {
	if _, ok := r.reconciler.(Finalizer); !ok {
		return resource, nil
	}
	if resource.GetDeletionTimestamp().IsZero() {
